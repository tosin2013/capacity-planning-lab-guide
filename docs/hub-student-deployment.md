...
| Students see 403 at Grafana route | Layer 1 ClusterRoleBindings missing | Run `bash scripts/provision-grafana-student-access.sh --count 5 --hub-kubeconfig <HKC>` |
...
| Student can log in but cluster dropdown only shows `local-cluster` | **Root cause (v1.x)**: `deploy-workshop.sh` did not call `provision-grafana-student-access.sh` after RHACM import, so `RoleBinding/workshop-obs-view-<user>` objects were never created for each managed cluster namespace. Fixed in v1.1+. Also possible if RBAC was applied after student's first login (session caching). | 1. Run `bash scripts/provision-grafana-student-access.sh --count 5 --hub-kubeconfig <HKC>`. 2. Each student must **log out of Grafana** (top-right avatar → Sign out) and log back in via the `workshop-students` IDP. A fresh session re-evaluates RBAC and all managed clusters appear in the dropdown. |
