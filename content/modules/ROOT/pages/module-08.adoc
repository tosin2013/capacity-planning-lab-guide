...
|6 OLSConfig
|Applies the cluster-scoped `OLSConfig` CR pointing to the LiteMaaS endpoint with model `granite-3-2-8b-instruct`.

|7 Wait for service
|Polls the `lightspeed-app-server` pod (label: `app.kubernetes.io/name=lightspeed-service-api`) until it is Running.
|===
