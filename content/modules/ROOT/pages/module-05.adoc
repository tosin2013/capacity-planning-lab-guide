= Module 5: Infrastructure Track - Fleet Architecture & Sizing
...
[TIP]
====
*Converting Ki to GB*: Divide by 1,048,576. `30988400 / 1048576 ≈ 29.5GB` allocatable.
Your value will vary slightly depending on the instance type and how much the OS and Kubernetes system components reserve.
====
...
On a 32GB node (e.g., `m5.2xlarge`):
* Allocatable: ~29.5GB (`30988400Ki`) — 32GB minus system-reserved memory for the OS and Kubernetes components
* Current usage at 42 pods: ~3.8GB (13% of 29.5GB)
...
====
*Success Criteria*:
* All nodes now show `500` for pod capacity
* UPDATED column in `oc get mcp master` is `True`
...
Now deploy the mass-scheduling test in a dedicated namespace (no resource quota):

[NOTE]
====
The `{namespace}` namespace has a ResourceQuota that caps total pods at 100, which would silently limit the density test.
The density test runs in its own temporary namespace so the 400-pod target is reachable without touching the workshop quota.
====

[source,bash,role=execute]
----
oc new-project density-test-ns
oc create deployment density-test --image=registry.access.redhat.com/ubi9/ubi-micro:latest --replicas=400 -n density-test-ns -- sleep infinity
oc set resources deployment density-test -n density-test-ns --requests=cpu=10m,memory=16Mi --limits=cpu=50m,memory=32Mi
----
...
[source,bash,role=execute]
----
watch -n 2 'oc get deployment density-test -n density-test-ns && echo "" && oc get pods -n density-test-ns --field-selector=status.phase!=Running | wc -l'
----
...
# Note: counts pods across all namespaces including density-test-ns
echo "Node: $NODE"
...
Remove the density test namespace (this also deletes all pods inside it):

[source,bash,role=execute]
----
oc delete project density-test-ns
----
...
[IMPORTANT]
====
*Wait for the MCO rollout to complete before starting Module 5.*

Deleting the `KubeletConfig` triggers the Machine Config Operator to roll back every worker node
one at a time. Each node is cordoned, drained, rebooted with the reverted config, then uncordoned.
On a 3-node cluster this takes *30–45 minutes*. On larger clusters it can take longer.

*If you start Module 5 while this rollout is in progress*, you will see nodes cycling through
`Ready,SchedulingDisabled` → `NotReady` → `Ready` — which is alarming and unexplained without this context.

Wait until the `worker` MachineConfigPool reports `UPDATED=true` and `DEGRADED=false`:
====

[source,bash,role=execute]
----
echo "Waiting for MCO rollback to complete (30-45 min)..."
oc wait mcp/master --for=condition=Updated=True --timeout=3600s
----
...
[NOTE]
====
You can also watch the progress node-by-node while waiting:
...
The `MACHINE COUNT`, `READY MACHINE COUNT`, and `UPDATED MACHINE COUNT` columns will converge.
All three should show the same number when the rollout is done.

Feel free to take a break — this is a good opportunity to review your notes from Parts 1–5 or read the Key Takeaways below.
====

== Lab 5 Summary: Density Decisions
...
In xref:module-06.adoc[Module 6: Fleet Observability with RHACM], you'll enable Multi-Cluster Observability, customize metric allowlists, and build a unified capacity dashboard that shows pod density, etcd usage, and node requirements across your entire fleet.
