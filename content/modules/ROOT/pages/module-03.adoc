...
load-generator   Deployment/load-generator   50%/75%   1         5         1          10s
----

At ~50% utilization (5m / 10m request), the HPA is watching but not yet scaling. The load-generator rests 60 seconds between 30-second test runs in normal mode, so average CPU stays below the threshold. Now simulate a traffic spike by enabling continuous stress mode:
...
----
NAME             REFERENCE                   TARGETS    MINPODS   MAXPODS   REPLICAS   AGE
load-generator   Deployment/load-generator   160%/75%   1         5         3          2m
----
...
```
Current CPU: ~16m sustained (continuous wrk -t8 -c500)
Request:     10m per pod
Utilization: 16m / 10m = 160% of request
Target:      75%
Desired Replicas: ceil(1 × 160 / 75) = ceil(2.13) = 3
```

HPA scaled from 1 → 3 replicas to spread the load and bring per-pod utilization down toward target.

After replicas stabilize:
```
Total CPU: ~16m distributed across 3 pods = ~5m per pod
Utilization: 5m / 10m = 50% ✓ Within target
```
====
