...
*Requests and limits are set, but intentionally minimal* — 10m CPU and 16Mi memory requests are far below what a real workload needs.
Because requests < limits, this pod has a *Burstable* QoS class, not BestEffort.
Under memory pressure it sits near the *bottom of the eviction stack*, just above a true BestEffort pod — making it one of the first Burstable pods to be evicted.
...
[NOTE]
====
*Fresh cluster? P95 values may be low.*

The right-sizer queries Prometheus for the *95th-percentile* CPU and memory usage over the past 7 days.
On a cluster deployed today the script automatically falls back to a 1-day window, but a single day may
still show only baseline idle usage rather than the peak load your application experiences under real traffic.

*What to expect and what to do*:

* The P95 numbers are correct for the data available — if the workload has only been running a few hours, the recommendation will be conservative (close to current idle usage).
* This is actually a useful teaching moment: right-sizing with too short a window gives you a request that is too low and will trigger throttling or OOMKills the first time real traffic hits.
* The `noisy-neighbor` deployment (scaled up in the next step) will spike CPU and give the right-sizer more signal. Rerunning after the noisy-neighbor test will show higher P95 values.
* In production, always run the right-sizer after the workload has experienced a representative traffic pattern — ideally 7+ days including at least one peak window.
====
...
[WARNING]
====
*Fresh cluster? Apply a memory safety floor before Part 5.*

On a cluster deployed today, the right-sizer's P95 memory recommendation may be very low (e.g. 8Mi) because the workload has not yet experienced stress traffic. Part 5 applies `TARGET_RPS=500` which significantly increases memory usage — a request of 8Mi will cause OOMKills before the HPA can observe a stable metric.

Run this after the right-sizer to enforce a minimum memory request of 64Mi while preserving the right-sizer's CPU recommendation:

[source,bash,role=execute,subs="+attributes"]
----
CURRENT_MEM_REQ=$(oc get deployment load-generator -n {namespace} -o jsonpath='{.spec.template.spec.containers[0].resources.requests.memory}')
CURRENT_MEM_REQ_MI=$(echo "${CURRENT_MEM_REQ}" | sed 's/Mi//')
if [ "${CURRENT_MEM_REQ_MI}" -lt 64 ] 2>/dev/null; then
  echo "Memory request ${CURRENT_MEM_REQ} is below 64Mi — applying safety floor"
  oc set resources deployment load-generator -n {namespace} \
    --requests=memory=64Mi --limits=memory=128Mi
  oc rollout status deployment/load-generator -n {namespace} --timeout=60s
else
  echo "Memory request ${CURRENT_MEM_REQ} is sufficient — no change needed"
fi
----

This is only needed on day-zero clusters. After 24+ hours of traffic, the right-sizer's memory recommendation will be high enough that no floor is necessary.
====
...
Verify the HPA for `load-generator` — it was pre-deployed by the workshop setup so it's already configured to scale at 75% CPU:

[NOTE]
====
The `load-generator` HPA is pre-deployed as part of the workshop sample applications.
The goal of this step is to *read and understand* the HPA configuration, not create it.
If you are running this in your own environment without the sample apps, create it with:
...
[source,bash,role=execute,subs="+attributes"]
----
NAMESPACE={namespace} ~/module-03/oom-demo.sh
----
...
== Module 3 Cleanup: Restore ArgoCD State

Before moving to Module 4, restore the `load-generator` deployment to its original resource settings.
Module 3 experiments changed CPU and memory values — if you leave those changes in place, ArgoCD will
show the deployment as *OutOfSync* and a future sync could reset your work mid-workshop.
...
[source,text]
----
deployment.apps/load-generator resource requirements updated
Waiting for deployment "load-generator" rollout to finish: 1 out of 1 new replicas have been updated...
deployment "load-generator" successfully rolled out
----

Confirm the resources are restored:

[source,bash,role=execute,subs="+attributes"]
----
oc get deployment load-generator -n {namespace} \
...
You should see: `requests cpu=10m memory=128Mi | limits cpu=200m memory=256Mi`

[NOTE]
====
*Why this matters*:

ArgoCD deployed `load-generator` with `requests: cpu=10m / memory=128Mi` and `limits: cpu=200m / memory=256Mi`.
The right-sizer experiments in Parts 3–5 changed these values based on observed P95 usage.

Restoring the original values here:

* Clears the ArgoCD *OutOfSync* state — no surprise resets later in the workshop.
* Gives Module 5's density calculations a consistent baseline.
* Leaves the cluster in the state ArgoCD expects as the source of truth.
====
...
In xref:module-04.adoc[Module 4: The Right-Sizing Activity], you'll put the right-sizing skills from this module into practice with a real-world ops-meets-dev scenario. You'll use Grafana dashboards to diagnose a deliberately mis-sized application and collaboratively fix it.

The skills from this module (accurate requests) directly feed into Module 5's capacity calculations. If requests are wrong, density calculations are wrong, and you'll buy the wrong amount of infrastructure.

== Further reading

* link:https://tosin2013.github.io/capacity-planning-lab-guide/blogs/post-d-kubernetes-requests-limits-explainer[Kubernetes requests and limits: why both teams are right] — Red Hat blog post expanding on QoS classes, CPU throttling, P95 right-sizing, and the HPA formula covered in this module.
