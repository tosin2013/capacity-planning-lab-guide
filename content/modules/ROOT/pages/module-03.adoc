...
* *CPU Request*: Set to 95th percentile usage (e.g. `10m` in this environment)
* *CPU Limit*: Set to 2× the request to allow bursts (e.g. `20m`)
* *Memory Request*: Set to 95th percentile + 20% buffer (e.g. `16Mi`)
* *Memory Limit*: Set to 95th percentile + 50% buffer (e.g. `24Mi`)
...
The right-sizer already printed the exact command you need — scroll up in your terminal to find the `Apply with:` block at the end of the load-generator right-sizer output (the run without `POD_SELECTOR`), then copy and paste that `oc set resources` command to apply it now.

[NOTE]
====
If you scrolled past the output, re-run the right-sizer to get the command again:
...
load-generator   Deployment/load-generator   <unknown>/75%   1         5         1          10s
----

[NOTE]
====
The TARGETS column shows `<unknown>` for a few seconds while the metrics server collects the first sample, then switches to a percentage. Once the pod has been running for ~30 seconds you can re-run `oc get hpa` and see a real value.
====

The HPA is watching but the load-generator is not yet under sustained stress. Now simulate a traffic spike by enabling continuous stress mode:
...
In stress mode (`TARGET_RPS≥500`) the load-generator runs `wrk` continuously — no sleep between runs — with 8 threads and 500 connections. Wait 90 seconds for the new pod to roll out and two wrk cycles to complete, then check HPA status:
...
```
Current CPU: ~14m sustained (continuous wrk -t8 -c500)
Request:     10m per pod  (from the right-sizer recommendation)
Utilization: 14m / 10m = 140% of request
Target:      75%
Desired Replicas: ceil(1 × 140 / 75) = ceil(1.87) = 2
```

HPA scaled from 1 → 2 replicas to spread the load and bring per-pod utilization down toward target.

After replicas stabilize:
```
Total CPU: ~14m distributed across 2 pods = ~7m per pod
Utilization: 7m / 10m = 70% ✓ Within target
```
====
