...
Scroll up in your terminal to find the `Apply with:` block at the end of the load-generator right-sizer output (the run *without* `POD_SELECTOR`), then copy and run that `oc set resources` command.

[NOTE]
====
If you scrolled past the output, re-run the right-sizer to refresh the recommendation:
...
After applying, verify the QoS class of the newly rolled-out pod:
...
You should see `Burstable` — the pod now has a request lower than its limit, which is the correct production pattern for a workload with variable CPU.
...
Copy and run the `Apply with:` command from the output.
...
```
Desired Replicas = ceil( 1 × actual% / target% )
```

For example:

* `140%/75%` → `ceil(1.87)` = 2 replicas
* `220%/75%` → `ceil(2.93)` = 3 replicas
* `60%/75%` → `ceil(0.80)` = 1 replica (no scale needed)

Does the REPLICAS count you observe match what the formula predicts? It should. If REPLICAS is still 1 and TARGETS is above 75%, wait another 30–60 seconds — HPA has a built-in stabilization window to prevent thrashing.
====

==== Experiment: Tune the request, observe the HPA response

The most effective way to build intuition is to deliberately change the request and predict the outcome *before* you run `oc get hpa`. Try each of these and verify your prediction:

[source,bash,role=execute,subs="+attributes"]
----
# Smaller request → HPA fires sooner, at lower load
oc set resources deployment load-generator -n {namespace} \
...
[source,bash,role=execute,subs="+attributes"]
----
# Larger request → HPA fires later, or not at all
oc set resources deployment load-generator -n {namespace} \
  --requests=cpu=200m --limits=cpu=500m
oc get hpa -n {namespace} load-generator
----

Each time, before running `oc get hpa`, write down your prediction: *given this request and the current CPU usage, what utilization percentage do I expect to see?* Then check if you were right. That prediction skill — reasoning about how request values affect scaling decisions — is what this lab is building.
