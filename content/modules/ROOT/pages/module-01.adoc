...
[source,textinfo]
----
besteffort-app-5598f76d5b-9jtz4    10m     16Mi
...
load-generator-67567c576-hdvkj     10m     128Mi
----

[NOTE]
====
The `besteffort-app` deployment manifest does *not* specify resource requests, but OpenShift namespaces with a *LimitRange* automatically inject default values (here, `10m` CPU / `16Mi` memory). This makes the pod *Burstable* rather than *BestEffort*. If your namespace has no LimitRange, you would see `none` / `none` and a true BestEffort QoS class.
====
...
* `besteffort-app` has minimal requests (LimitRange defaults) — it is still under-specified and at eviction risk
...
* link:https://tosin2013.github.io/capacity-planning-lab-guide/blogs/post-b-openshift-capacity-planning-explainer[OpenShift capacity planning: from reactive firefighting to predictive forecasting] — Red Hat blog post covering planning horizons, the allocated vs. consumed gap, and the Pod Velocity Model introduced in Modules 1 and 2.
