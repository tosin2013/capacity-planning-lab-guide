...
[NOTE]
.Compact & SNO Clusters: Verify the MCP target
====
On *3-node compact clusters* and *SNO*, every node has both `master` and `worker` roles, but the MCO assigns them to the `master` MachineConfigPool — the `worker` pool has 0 machines. If you apply a KubeletConfig that targets `worker`, it generates a MachineConfig but applies it to no nodes, so `maxPods` will not change.

Check which pool your nodes actually belong to:
...
If `worker` shows `MACHINECOUNT: 0`, re-apply the KubeletConfig targeting the `master` pool instead:
...
[WARNING]
On a compact or SNO cluster this will trigger a reboot of *all* nodes (they are also control-plane nodes). Expect 10–15 minutes of API unavailability while the single node restarts.
====
...
----
oc create deployment density-test --image=registry.access.redhat.com/ubi9/ubi-micro:latest --replicas=400 -n {namespace} -- sleep infinity
